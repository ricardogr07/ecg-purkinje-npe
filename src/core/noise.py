...
seed. It supersedes the earlier relative model (5% of each lead's std), which flattered
...
# Physiological-mV calibration. The 1/|r| pseudo-ECG synthesis emits uncalibrated
...
# Retired relative model, kept for the historical smoke test only.
...
def add_waveform_noise(ecg: np.ndarray, noise_frac: float, rng: np.random.Generator) -> np.ndarray:
    """Retired: per-lead Gaussian, sigma_lead = noise_frac * std(lead). Use the
