...
The forward operator maps a conduction parameter vector $\theta$ to a 12-lead surface signal through a fixed chain: a `purkinje-uv` fractal-tree His-Purkinje network, a fast iterative method (FIM) eikonal activation solve on that tree, and a pseudo-ECG readout. The pipeline runs at fixed anatomy on a single fixed demonstration geometry, the `crtdemo` biventricular mesh (geometry id `cardiac_demo`) bundled with the `myocardial-mesh` library, which is a demonstration rig rather than a patient-derived mesh and is distinct from the Strocchi cohort; anatomy is not inferred. The same pipeline is separately shown to ingest real public four-chamber meshes from the Strocchi et al. cohort [@strocchi2020] as a method-generality demonstration, with no identifiability claim on those hearts (Section 4.5).
...
Each engineered feature is typed by physical unit, and the type sets which feature-channel sigma applies (Section 3.4): amplitude features are millivolt peak or wave voltages (for example an early V1 to V2 R-wave amplitude), and timing features are millisecond durations and interlead delays (feature_classification_note, L-B7F). This class-level typing is fixed by the contract's amplitude-versus-timing split (contract_b_OBSERVATION_MODEL; feature_classification_note, L-B7F). The exact fifteen-item feature vector, enumerated below from the feature-extraction module (`src/core/features.py`), is 13 amplitude features and 2 timing features.

The fifteen features, in canonical order, are: twelve per-lead peak-to-peak amplitudes (`p2p_lead0` to `p2p_lead11`, each the maximum minus the minimum of one lead over the trace, in mV, amplitude type); the peak of the twelve-lead root-sum-square vector magnitude (`vecmag_peak`, in mV, amplitude type); the active-window fraction (`qrs_active_frac`, the fraction of samples for which the vector magnitude exceeds 5 percent of its peak, dimensionless, timing type); and the fractional time to peak (`time_to_peak_frac`, the argmax of the vector magnitude divided by the trace length, dimensionless, timing type). This is 13 amplitude features and 2 timing features. Both timing features are fractions of the trace length; to convert to milliseconds, multiply by the trace duration, which is how the feature-channel Cramer-Rao bound maps the 5 ms timing floor to a fraction (Section 4.3).
...
The per-parameter conformal recalibration moves the joint posterior from a pre-conformal TARP ATC of -0.0565 (underdispersed, that is, overconfident, in the sbi sign convention) to a post-conformal joint TARP ATC of +0.0072 (nominal, marginally conservative); the contraction-versus-budget behavior is reported in the results (Section 4.2).
...
Against the waveform noise floor (white Gaussian $\sigma = 0.025$ mV per sample
per lead, applied before feature extraction), the prior-averaged contraction
spectrum orders the seven parameters as follows (smaller contraction is more identifiable)
(L-B7F, parameter\_to\_feature\_map):

| Rank | Parameter | Contraction (against the waveform floor) | ECG feature it moves |
|---|---|---|---|
...
against the waveform noise floor (white Gaussian sigma = 0.025 mV per sample per lead):
...
least amplitude-confounded quantity in the QRS, a timing feature robust to amplitude noise, so
...
the prior and they are diffuse against the waveform noise floor (white Gaussian sigma = 0.025 mV per sample per lead) (L-B7F, parameter\_to\_feature\_map)
...
against the waveform noise floor (white Gaussian sigma = 0.025 mV per sample per lead) only the RV extent contracts (about 0.63) while the LV extent
...
about 1.0 to 1.2 against the waveform floor) is a property of this fixed anatomy
...
the calibration result backwards. The frozen per-parameter KS p-values (before to after conformal) are cv 0.031 to 0.111, delta_iv 0.444 to 0.613, init_length_lv 0.0043 to 0.630, init_length_rv 0.055 to 0.055, branch_angle 0.00002 to 0.023, w 0.00035 to 0.413, and cv_myo 0.00007 to 0.150, a post-conformal median of 0.150. Before recalibration several parameters fail the KS test at 0.05 (init_length_lv, branch_angle, w, cv_myo, with cv marginal); after recalibration all clear 0.05 except branch_angle (0.023, still a roughly thousandfold improvement), and init_length_rv is unchanged because its inflation factor is one. The per-parameter contraction values (pre-conformal to post-conformal) are cv 0.559 to 0.671, delta_iv 0.137 to 0.151, init_length_lv 0.799 to 1.039, init_length_rv 0.628 to 0.628, branch_angle 0.804 to 1.206, w 0.793 to 1.070, and cv_myo 0.265 to 0.345. Conformal inflation loosens every contraction, as it must when correcting overconfidence, and the constraint-versus-diffuse ordering survives; init_length_rv is unchanged because its inflation factor is one.
...
The post-conformal per-parameter contraction is reported as a function of the
...
$N = 5000$. The curve is measured at two budgets, $N = 1000$ and $N = 4000$, with three seeds each, so it is a two-point trend rather than a converged curve. Between these budgets the post-conformal median contraction keeps tightening for cv (0.767 to 0.491), delta_iv (0.190 to 0.147), init_length_lv (1.248 to 1.010), and init_length_rv (0.581 to 0.413), the trend exceeding the across-seed spread for cv and init_length_lv and comparable to it for the other two, so these parameters are budget-responsive over this range; branch_angle (0.976 to 1.047), w (1.026 to 1.108), and cv_myo (0.300 to 0.326) do not tighten, their change lying within or below the seed spread, so they are not budget-responsive over this range.
What it is designed to show, against the waveform noise floor (white Gaussian sigma = 0.025 mV per sample per lead): if contraction for the diffuse block
...
`init_length_lv`, consistent with its diffuse contraction against the waveform noise floor (white Gaussian $\sigma = 0.025$ mV per sample per lead)
(R-02).
...
**Single fixed anatomy.** All identifiability results use one fixed geometry, the `crtdemo` biventricular demonstration mesh (geometry id `cardiac_demo`, a demonstration rig rather than a patient-derived heart, distinct from the Strocchi cohort), and amortization is over conduction parameters at fixed geometry, not over anatomy (L-B5-02). The contraction ordering, degeneracy structure, and per-parameter verdicts are therefore anatomy-specific and are not claimed to generalize across hearts; inter-subject variability in chamber size, orientation, and torso pose is outside the current scope (L-B5-02).
...
**What would confirm it.** Re-running the characterization under a bounded torso forward and observing that the contraction and CRLB of `init_length_rv` (against the waveform noise floor (0.025 mV per sample per lead)) worsen (move toward 1) by a strictly larger factor than those of both `delta_IV` and `cv_myo` at the same floor (L-B7F-01).
...
3. The FIM-versus-posterior gap being a real estimator limitation: the current headline reads the diffuse contraction of `branch_angle` (about 1.21), `w` (about 1.07), and `init_length_lv` (about 1.04), all against the waveform noise floor (white Gaussian sigma = 0.025 mV per sample per lead), together with a locally well-conditioned FIM (condition number 18.3, CRLBs above the noise floor), as a prior-width effect on contraction plus a genuine near-null direction along `init_length_lv`, not as the estimator failing to reach the information the FIM reports (L-B6E). If a controlled check (a larger training budget than the working N, a different density estimator, or an amortization-gap diagnostic) shows the posterior is systematically wider than the FIM-derived CRLB allows, the gap is a real estimator limitation and part of the reported diffuseness is an estimator shortfall rather than an intrinsic identifiability property; the contraction-versus-N check (Section 4.2) shows init_length_lv tightening with budget, consistent with an estimator or budget limit for that parameter, while branch_angle and w do not respond over the measured range (L-B6E).
...
**Anatomy.** The identifiability study runs on a single fixed geometry, the `crtdemo` biventricular demonstration mesh (geometry id `cardiac_demo`) bundled with the `myocardial-mesh` library; it is a demonstration rig, not patient-derived, and is not one of the Strocchi meshes. Separately, the method-generality demonstration (Section 4.5) ingests real public four-chamber meshes from the cohort of Strocchi et al., PLoS ONE 2020, DOI 10.1371/journal.pone.0235145, PMID 32589679, archived on Zenodo (record 3890034, DOI 10.5281/zenodo.3890034) under the Creative Commons Attribution 4.0 International (CC-BY-4.0) license [@strocchi2020]; that cohort comprises 24 four-chamber heart meshes, one per heart-failure patient, each archived as a compressed folder (NN.tar.gz, NN in 01 to 24) with both a coarse 1.1 mm and a fine 0.39 mm mesh, and the demonstration uses the coarse 1.1 mm meshes of hearts 01 and 02. No new patient data were generated in this work, and no real patient data are used as an inversion target anywhere in the project; the inversion target is simulator output (R-03).
...
2. **Simulation sweep.** A single seeded set of 5000 draws, stored per sample with both the engineered ECG features and the full 12-lead waveforms, with a held-out test split, so the paired features-versus-waveform comparison costs training time, not simulation time. The same draws are released as two files that differ only in row order: `day3_7d_snr_ckpt.npz` (the headline features sweep, whose ordering the shipped posteriors use) and `day4_wave_ckpt.npz` (features and full 12-lead waveforms); both are in release `v0.1.0-submission` (https://github.com/ricardogr07/ecg-purkinje-npe/releases/tag/v0.1.0-submission), so both observation models regenerate exactly from the released data (L-B5-07).
3. **Trained weights.** The NPE checkpoint is released alongside the inference code so the posteriors regenerate without re-training: `release_posterior.pt` and `release_posterior.json` in release `v0.1.0-submission`, which reproduce the shipped posteriors bit-for-bit via `experiments/emit_headline_checkpoint.py` (L-B5-07).
4. **Container.** A container pinning the software stack (purkinje-uv for the fractal-tree Purkinje generation and FIM eikonal solver, sbi for NPE, SBC, expected coverage, and TARP) so the environment is reproducible. The image (5.76 GB) exceeds GitHub's per-file release limit and is not attached; it builds from source per `REPRODUCE.md` against the tag's pinned `Dockerfile.*` and `uv.lock`. Recorded build digests: CLI `conduction-lens` sha256:4ba109d7d9926b07c485a5832de239a24c52381a0135e1b6a69150187bda5444, demo `conduction-lens-demo` sha256:b020ecf69272b7fe143ecd17fa3485fc4f0c2d0727f5056f369c07b50d963c1b (build ids vary by machine and date; build from the pinned files for byte-stable reproduction) (L-B5-07).
5. **Calibration artifacts.** The SBC ranks, expected-coverage and TARP outputs, and the seeds used, released so the calibration claim is independently checkable: `hl_tarp_results.json` in release `v0.1.0-submission` carries the posterior, SBC ranks, coverage curve, and pre/post TARP (post-conformal joint TARP ATC = +0.0072) (L-B5-07).
...
| L-B7M-17 | Prediction 1 outcome (init_length_rv degrades more than delta_IV/cv_myo under bounded forward) | forward reasoning; bounded-forward run NOT executed | ASSE