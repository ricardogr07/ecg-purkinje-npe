...
b.text(len(names) - 0.5, 0.07, "p=0.05", ha="right", fontsize=6.5, color=C_DIFF)
b.set_xticks(xx)
...
fig.tight_layout()
fig.savefig(f"{OUT}/fig_calib_wide.png")
plt.close(fig)
...
a.set_ylabel("post-conformal contraction")
a.set_xticks(Ns)
a.set_xlim(Ns[0] - 300, Ns[-1] + 1600)
a.set_ylim(0, 1.3)
a.set_title("(a) contraction vs budget")
a.text(
    0.03, 0.05, "green tightens, red flat/widens", transform=a.transAxes, fontsize=6, color="0.35"
)
b = ax[1]
...
fig.tight_layout()
fig.savefig(f"{OUT}/fig_bc_wide.png")
plt.close(fig)
print("wide figures done")
