...
- Eligibility, two issues. (1) Part-count drift: execution-plan and brief say a 4-part organizer question, state.md now says 5-part; reconcile. (2) Bigger exposure: the 4-part question does not cover `myocardial-mesh`, the vendored library that actually computes the 12-lead ECG, whose upstream repo `purkinje-learning-myocardial-mesh` shares the name of the forbidden `purkinje-learning`. A sharp organizer will ask whether the ECG engine is effectively thesis code. This must be named head-on in the posted question.
