...
## Update, Day-2 evening part 2 (A forward fidelity SOLVED, B calibration fixable), parallelized
Ran A (foreground) and B (background) together; both resolve positively.
- **A, forward fidelity (`experiments/forward_fidelity.py`): the simulator is sound; it was a scenario mismatch.** nb-parity is corr 1.000 (ECG synthesized from the true activation matches `True_ecg` exactly, our lead-field is exact). The corr-0.20 gap came from `REFERENCE_THETA` using `delta_iv=0`; `True_ecg` carries a ~-75 ms interventricular delay. Scanning delta_iv, the match rises to corr 0.63 raw and 0.86 time-aligned near delta_iv -75 to -100. The residual is a global time-offset (alignable) plus minor coupling differences.
- **Knock-on (Contract A): the true `delta_iv` (~-75 ms) is OUTSIDE the provisional prior [-40, 40].** The prior cannot represent the true value, so it must be widened (recommend about [-90, 40] or [-100, 100]) before the Thu freeze, and `REFERENCE_THETA`'s delta_iv should be ~-75. This likely also contributed to the miscalibration.
- **B, calibration (`experiments/calib_fix.py`): overconfident by ~1.3x, fixable post-hoc.** Post-hoc posterior variance inflation by t=1.3 calibrates SBC (ks 0.017 -> 0.242); t>=1.6 over-corrects (ks -> 0.008 then 0.000); an ensemble of 5 alone does not fix it (ks 0.014). So the flow is right-location but too narrow by ~30%. Adopt conformal calibration (pick the inflation with a coverage guarantee) rather than a hand-tuned factor.
- **Net:** forward validated, calibration has a concrete cheap fix, and the delta_iv prior needs widening. Next: widen the delta_iv prior, set REFERENCE delta_iv ~-75, re-sweep, and wrap the NPE in conformal calibration, then re-check SBC/TARP and the contraction spectrum.
