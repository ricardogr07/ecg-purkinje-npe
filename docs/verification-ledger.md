...
| Post-conformal joint TARP (headline split): joint ATC moves -0.057 pre-conformal to +0.007 post-conformal (SBC ks 0.15); a cross-split re-run gives -0.086 to +0.024 | in-kernel emit on the headline checkpoint, matched sample sets (only the per-parameter inflation differs); reproduced | VERIFIED (reproduced): per-parameter conformal brings the JOINT into approximate calibration, not just the marginals |
| F8 ordering robustness: the identifiable-4 stay below the diffuse-3 at sigma 0.10 mV (4x noise) and at 2.0 mV amplitude, both seeds, pre-registered | in-kernel retrains on the stored waveforms (no new sims); pre-conformal contraction on the F3 split | VERIFIED (reproduced): the identifiable/diffuse ordering is not an operating-point artifact |
