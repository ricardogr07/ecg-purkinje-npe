...
The scientific source of truth: problem, method, priors and their provenance, prior art, and the
...
**The finding:** the **quantified identifiability spectrum + degeneracy map**, deliberately NOT the a-priori-obvious "macro identifiable / micro not":
...
5. **The headline table:** parameter × contraction × coverage. **The headline figure:** posterior corner/degeneracy plot with a "resolved vs. unresolved" reveal.
...
8. Packaged repo (tests, CI), Dockerized, demo page; technical write-up (problem → method → results → honest limitations → future work).
...
> **Freeze note:** Contract A froze at **7D**, adding `cv_myo` (myocardial conduction velocity, inferred over [0.5, 1.0], Fu 2024) as the 7th parameter so nothing is fixed to flatter the result. Full reconciled priors, sources, and the `delta_iv` provenance are in **Appendix A**. The block hypothesis below is unchanged.
...
Priors: physiological uniform ranges. 8D extension if time: `fascicles_angles` (LV/RV) → block 1; `l_segment` → block 2.
...
- Punchy reveal: **"this parameter is resolved, this one is unresolved at the stated noise floor"**, grounded in the measured contraction table.
- **Calibration panel** (coverage curve) = "findings you can trust."

---

## 7. Prior-art verification ledger
...
| `purkinje-uv` tree growth is stochastic (PCG64 nuisance) | GitHub source: `config.py`, `branch.py`, `fractal_tree.py`, `edge.py`, `nodes.py`, `mesh.py` | ✗ rng not consumed in growth; branch angle read directly from θ → deterministic-given-θ; confirmed by a same-θ runtime diff |
...
The Contract A freeze, grounded in public literature and arrived at independently of the thesis `BOECGParameter` bounds.
...
|---|---|---|---|---|---|---|
| `cv` | CV_pk | m/s | [1.3, 3.5] | 2.2 | measured | Maguy 2009 (control 2.2 m/s, 1.5 in CHF); floor lowered 1.5 -> 1.3 to bracket crtdemo true ~1.4 interior |
...
| `cv_myo` | CV_myo | m/s | [0.5, 1.0] | 0.67 | measured | Fu 2024. Inferred as the 7th parameter |

**Frozen: 7 parameters** (`cv_myo` is inferred rather than fixed, so no fixed nuisance remains). `cv_myo` is appended last so the first six keep their canonical order. Robustness supplement: also report at `cv_myo` = 0.5 and 1.0 to show conclusions do not hinge on it.
...
## Freeze decisions
1. **`init_length` LV vs RV: SEPARATE, both [30, 60].** Update `src/core/theta.py` from its old asymmetric guess (LV [15,45], RV [45,95]) to symmetric [30,60] on each. Labeled our modeling choice (Tanikella fixed 50 mm, LV only).
2. **One CV or two: SEVEN params, infer `cv_myo` over [0.5, 1.0].** More defensible (nothing fixed to flatter the result); costs calibration budget, so use the 5k sweep on the 7D contract. Report `cv_myo` = 0.5 / 1.0 slices as robustness.
3. **`delta_iv` width, now load-bearing.** The crtdemo reference is a dyssynchronous CRT case with a true `delta_iv` of about -75 ms, which is OUTSIDE the earlier [-25, 25] and [-40, 40] boxes, so the prior literally could not represent the truth and fed the miscalibration. The operative prior must span the dyssynchrony regime. Widening is a bug fix, not a preference. Two cautions: (a) prefer a SYMMETRIC sourced box (e.g. [-90, 90] or the sourced BBB range from Research P2.6) over an asymmetric box centered on -75, which would look like tuning the prior to the answer; (b) the width moves the headline, since contraction = post_std / prior_std, a wider `delta_iv` prior mechanically shrinks its contraction, so choose the width on physiology and document it. Working value: [-90, 40].
...
- The earlier gap (Sahli Costabal 2016 table unread behind a captcha) is now **closed**: the model-parameter ranges trace to Tanikella 2025 Table 1 (independent third party, read at full text) plus Sahli Costabal 2015 base values. Measured CVs from Maguy 2009 and Fu 2024.
...
- Full source ledger with DOIs/PMIDs is in `docs/verification-ledger.md`.
...
An earlier build used 5% per-lead **relative** Gaussian noise, which review flagged as flattering identifiability (it preserves near-silent leads almost exactly). This Contract D replaces it with an **absolute-mV** model sourced from measured ECG reproducibility. Switch Code to this at the freeze.
...
Full checked-vs-asserted ledger: `docs/verification-ledger.md`.
